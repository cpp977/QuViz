...
void Cylinder::draw(Eigen::Matrix<double, Eigen::Dynamic, 3>& V, Eigen::Matrix<int,Eigen::Dynamic,3>& F, Eigen::Matrix<double,Eigen::Dynamic,3>& C) const
{
        std::size_t N=30;
        ...
        ps_g[0] = -0.5*Eigen::Vector3d::UnitZ();
        ps_g[1] = r*Eigen::Vector3d::UnitY()-0.5*h*Eigen::Vector3d::UnitZ();
        for (std::size_t n=2; n<N+1; n++) {
                ps_g[n] = Eigen::AngleAxisd(rot_angle, Eigen::Vector3d::UnitZ()) * ps_g[n-1];
        }
        ...
        for (std::size_t n=2; n<N+1; n++) {
                ps_r[n] = Eigen::AngleAxisd(rot_angle, Eigen::Vector3d::UnitZ()) * ps_r[n-1];
        }

        for (std::size_t n=0; n<N+1; n++) {
                V.row(curr_size_V+n) = ps_g[n];
        }
        for (std::size_t n=0; n<N+1; n++) {
                V.row(curr_size_V+N+1+n) = ps_r[n];
        }
        ...
        if (DRAW_BASES) {
                ...
                for (std::size_t n=0; n<N-1; n++) {
                        F.row(curr_size_F+n) << curr_size_V+0, curr_size_V+1+n+1, curr_size_V+1+n;
                }
                ...
                for (std::size_t n=1; n<N; n++) {
                        F.row(curr_size_F+N+n) << curr_size_V+n+1,curr_size_V+N+2+n,curr_size_V+n+1-1;
                }
        }
